...
- 在计算机中，一个词（word）是由一组位（bits）组成的。这些位可以看作是一个二进制数字的向量。
- 在RISC-V架构下，一个词的长度可以是32位或64位。这其中，32位包含了四个字节（bytes）。

常规的实现机制（Generic Implementation）

- 使用程序计数器（Program Counter，PC）链接到指令地址。程序计数器是一个寄存器，它用来存储正在执行的指令的地址。
- 从内存中获取（fetch）指令。这个步骤通常由指令取取器（Instruction Fetcher）完成。
- 指令将告诉计算机需要执行什么操作。这些操作可能涉及到数据的获取、数据的处理、数据的储存等。
- 算术逻辑单元（Arithmetic Logic Unit，ALU）将执行指定的算术操作。这种操作可能包括加法、减法、乘法、除法等。
